...
BOOST_AUTO_TEST_CASE( testPortTaskInterface )
{
    ...
    // This is not supported behavior, as it will 'overtake' ownership,
     {
#ifdef RTT_USE_CPP11
        std::unique_ptr<TaskContext> tc1(new TaskContext( "tc", TaskContext::Stopped ));
        std::unique_ptr<TaskContext> tc2(new TaskContext( "tc2", TaskContext::Stopped ));
#else
        std::auto_ptr<TaskContext> tc1(new TaskContext( "tc", TaskContext::Stopped ));
        std::auto_ptr<TaskContext> tc2(new TaskContext( "tc2", TaskContext::Stopped ));
#endif
...
#ifdef RTT_USE_CPP11
    std::unique_ptr<InputPortInterface> reader(dynamic_cast<InputPortInterface*>(wp1.antiClone()));
#else
    std::auto_ptr<InputPortInterface> reader(dynamic_cast<InputPortInterface*>(wp1.antiClone()));
#endif
